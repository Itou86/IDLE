...
// 创建测试用的游戏状态
function createIdleState(points = 100) {
    return {
        points: points,
        shards: 10,
        stage: 1,
...
test('IdleSystem - 点击获得系统点', () => {
    const state = createIdleState(100);
    const earned = IdleSystem.click(state);
    Assert.equal(earned, 2, '基础点击应获得2系统点');
    Assert.equal(state.points, 102, '系统点应增加2');
    Assert.equal(state.stats.pointsTotal, 102, '系统点总计应增加2');
});
...
test('IdleSystem - 购买升级B', () => {
    ...
    Assert.true(result.success, '应有足够系统点购买升级B');
    Assert.equal(result.newLevel, 1, '等级应为1');
    Assert.equal(result.newValue, 2, '每秒收益应为2');
    Assert.equal(state.points, 80, '应扣除20系统点');
    Assert.equal(state.idle.autoLevel, 1, '状态应记录等级');
});

test('IdleSystem - 系统点不足无法购买升级B', () => {
    const state = createIdleState(15);
    const result = IdleSystem.buyAutoUpgrade(state);
    Assert.false(result.success, '系统点不足应失败');
    Assert.equal(state.points, 15, '系统点不应变化');
});
...
test('IdleSystem - 获取每秒自动收益', () => {
    const state = createIdleState(100);
    // 无idle状态时使用默认值，但BASE_AUTO_POINTS=1
    Assert.equal(IdleSystem.getAutoPointsPerSecond(state), 1, '初始应为1（基础自动收益）');

    state.idle = { clickLevel: 0, autoLevel: 3 };
    Assert.equal(IdleSystem.getAutoPointsPerSecond(state), 4, '3级应为4/秒 (1+3)');
});

test('IdleSystem - 离线收益计算', () => {
    const state = createIdleState(100);
    state.idle = { clickLevel: 0, autoLevel: 2 };  // 基础1 + 2 = 3系统点/秒
    state.stats.lastSaveTime = Date.now() - 5000;  // 5秒前

    const result = IdleSystem.calculateOfflinePoints(state);
    Assert.equal(result.points, 15, '5秒离线应获得15系统点 (3*5)');
...
test('IdleSystem - 离线收益上限8小时', () => {
    const state = createIdleState(100);
    state.idle = { clickLevel: 0, autoLevel: 1 };  // 基础1 + 1 = 2系统点/秒
    state.stats.lastSaveTime = Date.now() - 10 * 3600 * 1000;  // 10小时前

    const result = IdleSystem.calculateOfflinePoints(state);
    Assert.equal(result.seconds, 8 * 3600, '最多计算8小时');
    Assert.equal(result.points, 16 * 3600, '8小时应获得57600系统点 (2*28800)');
...
test('IdleSystem - 应用离线收益', () => {
    ...
    const result = IdleSystem.applyOfflinePoints(state);
    Assert.equal(result.points, 6, '应获得6系统点 (2*3)');
    Assert.equal(state.points, 106, '系统点应增加');
    Assert.equal(state.stats.pointsTotal, 106, '总计应增加');
});

test('IdleSystem - 离线收益为0时不变化', () => {
    const state = createIdleState(100);
    state.idle = { clickLevel: 0, autoLevel: 0 };  // 基础1 + 0 = 1系统点/秒
    state.stats.lastSaveTime = Date.now() - 3000;

    const result = IdleSystem.applyOfflinePoints(state);
    // 即使autoLevel=0，BASE_AUTO_POINTS=1，所以还是有收益
    Assert.equal(result.points, 3, '基础自动收益为1/秒，3秒应获得3');
    Assert.equal(state.points, 103, '系统点应增加3');
});
...
test('IdleSystem - getInfo 返回完整信息', () => {
    ...
    Assert.true(info.canAffordClick, '100系统点应够买点击升级');
    Assert.true(info.canAffordAuto, '100系统点应够买自动升级');
});

test('IdleSystem - getInfo 系统点不足时', () => {
    const state = createIdleState(15);
    const info = IdleSystem.getInfo(state);
    Assert.true(info.canAffordClick, '15系统点够买点击升级（5）');
    Assert.false(info.canAffordAuto, '15系统点不够买自动升级（20）');
});
...
test('IdleSystem - 处理无idle状态', () => {
    ...
    Assert.equal(IdleSystem.getAutoPointsPerSecond(state), 1, '无idle状态应为1（基础自动收益）');
