...
**⚠️ 在回答任何开发相关的问题或执行任何代码修改之前，必须先读取 `docs/roadmap.md`**，确认当前任务状态、优先级和阻塞项。

按顺序阅读：
1. **本文档**（CLAUDE.md）— 项目结构、代码规范、设计意图
2. **`docs/roadmap.md`** — 当前任务、优先级、已知问题 ← **每次会话先读**
3. **`docs/codemap.md`** — 定位相关代码模块和函数

### 2. 开发流程

**核心原则**：设计决策权属于用户，AI 负责执行。涉及设计选择（改什么、怎么改）必须先确认方案，只有纯执行（按已定方案写代码）才能直接动手。
...
用户提需求
    ↓
【AI】读取配置 → 读取系统 → 读取测试 → 分析需求 → 提出方案
    ↓
【用户】确认方案 / 提出修改意见
    ↓
【AI】写/更新测试用例 → 运行测试确认失败(RED)
    ↓
【AI】修改代码 → 运行测试确认通过(GREEN)
    ↓
【AI】考虑重构 → 运行全部测试 npm test
```

#### 什么时候必须先确认方案

以下情况**必须**先提出方案、等待用户确认，不能直接写代码：

| 场景 | 示例 |
|------|------|
| 涉及设计决策 | "这个系统该怎么改"、"选方案A还是方案B"、"数值怎么定" |
| 修改影响 >2 个文件 | 改 cards.js + stats.js + battle.js + 测试 |
| 用户主动说"讨论一下" | 任何用户明确要求先讨论的情况 |
| 重命名/重构现有内容 | 改41张卡牌名称、改世界名称、改成就命名 |
| 新增系统模块 | 新增一个完整系统（如排行榜、赛季） |
| 用户表述模糊 | "完善一下世界观"、"优化一下战斗"（需要拆解具体做什么） |
...
| 场景 | 示例 |
|------|------|
| 用户明确说"按方案X执行" | 方案已定，AI 只做代码实现 |
| 单文件、单行的明显修复 | 修复 typo、补漏引用的文件、调错一个数值 |
| 用户说"直接改" | 明确授权跳过确认 |
| 文档同步（代码已改完） | 按规则更新 codemap/roadmap |

#### 确认方案的标准格式

提出方案时应包含：

1. **需求拆解**：把用户的一句话需求拆成具体任务
2. **关键决策点**：列出需要用户决定的事项（选A还是B、数值定多少）
3. **影响范围**：会改哪些文件、是否影响存档兼容、测试是否需要更新
4. **建议方案**：给出推荐选项及理由

示例：

> **需求**：完善世界观，改成实际存在的作品世界
>
> **拆解**：
> - 世界1「混沌试炼」→ 改成具体IP
> - 世界2「魔法纪元」→ 改成具体IP
...
>
> **关键决策（需要你确认）**：
> 1. 世界1选什么IP？建议「生化危机1」（经典生存恐怖，无限流常见）
> 2. 世界2选什么IP？建议「哈利波特与魔法石」（魔法题材）
> 3. 世界3选什么IP？建议「进击的巨人」（热血战斗题材）
>
> **影响范围**：cards.js（41张卡name/desc + 3世界名 + 8套装名）、stages.js、测试描述文字
>
> 请确认IP选择，或给出你希望的IP。
...
│   ├── config/
│   │   ├── cards.js        # CARD_CONFIG：41张卡 + 8套装（3个世界）
...
```javascript
// 所有系统方法返回统一结构
{ success: boolean, reason?: string, errorCode?: string, ...data }

// 示例
if (gameState.shards < this.COST.shards) {
